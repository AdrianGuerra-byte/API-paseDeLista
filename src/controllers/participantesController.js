...
export const getParticipantes = async (req, res) => {
  try {
    const result = await db.execute('SELECT id, nombre, institucion, grupo, asistio FROM participantes');
...
// PUT actualizar participante completo
export const updateParticipante = async (req, res) => {
  try {
    ...
    await db.execute({
      sql: 'UPDATE participantes SET nombre = ?, institucion = ?, grupo = ?, asistio = ? WHERE id = ?',
...
// PATCH actualizar participante parcial
export const patchParticipante = async (req, res) => {
  try {
    ...
    await db.execute({
      sql: `UPDATE participantes SET ${fields.join(', ')} WHERE id = ?`,
      args: values
    });
...
export const deleteParticipante = async (req, res) => {
  try {
    ...
    res.json({ message: 'Participante eliminado', id });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
